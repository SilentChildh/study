...
**git log**：可看到详细版本信息，且看到提交者、提交日期。可以查看提交历史，以便确定要回退到哪个版本。

**git reflog**：（可显示前七位的精简版本号）查看命令历史，以便确定要回到未来的哪个版本。
...
- git diff --cached查看**暂存区和仓库**差异，如果此时暂存区为空，那么稍微有点不同，即：
...
- git diff HEAD 查看**工作区和仓库**的差异，其中：HEAD代表的是最近的一次commit的信息。
